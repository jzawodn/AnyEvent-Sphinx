...
sub serialize {
    ...
    $req .=
      pack("NNN", $self->{_cutoff}, $self->{_retrycount}, $self->{_retrydelay});
...
sub SetFilterRange {
    ...
    croak("SetFilterRange: attribute is not defined")
      unless (defined $attribute);
    croak("SetFilterRange: min ($min) is not an integer")
      unless ($min =~ m/$num_re/);
    croak("SetFilterRange: max ($max) is not an integer")
      unless ($max =~ m/$num_re/);
    croak("SetFilterRange: min ($min) value should be <= max ($max)")
      unless ($min <= $max);
...
sub SetFilterFloatRange {
    my ($self, $attribute, $min, $max, $exclude) = @_;
    croak("SetFilterFloatRange: attribute is not defined")
      unless (defined $attribute);
    croak("SetFilterFloatRange: min ($min) is not numeric")
      unless ($min =~ m/$num_re/);
    croak("SetFilterFloatRange: max ($max) is not numeric")
      unless ($max =~ m/$num_re/);
    croak("SetFilterFloatRange: min ($min) value should be <= max ($max)")
      unless ($min <= $max);
